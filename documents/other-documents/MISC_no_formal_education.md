...
- Synchronisation between devices requires complex conflict resolution (CRDTs, operational transforms).
...
When I needed to synchronise notebooks across devices, I did not implement a complex conflict resolution algorithm. I asked: _"How can I make sync simple and deterministic?"_

The answer was UUID‑level synchronisation: each commit changes exactly one UUID. Conflicts are resolved by timestamp – the newer chain wins. The result is a linear history with no merge commits, no manual conflict resolution, and no user‑visible complexity.

I did not ask whether these choices were “best practices.” I asked whether they worked. They did.

Only later did I learn that my choices contradicted many established patterns. But by then, the system was already running. It had survived crashes, power losses, USB removals, cross‑machine imports, and conflicting sync operations. It had never lost data. The contradictions were not weaknesses; they were evidence that the system operated under different assumptions.
...
The sync mechanism, in particular, emerged from a simple observation: _"Each commit changes exactly one UUID. If I group commits by UUID, I can compare timestamps and keep the newer chain."_ This was not a design pattern I had read about. It was a logical consequence of the UUID permanence that already existed.

The system is complex, but not because I made it complex on purpose. It is complex because the problems it solves (portable encryption, cross‑machine recovery, Git‑based resurrection, hardware binding without TPM, deterministic sync) are inherently complex. The complexity is in the relationships between components, not in the individual components themselves.
...
| Need to sync across devices without conflicts | Built UUID‑level sync based on timestamps                 |
...
Because I had never studied standard architectures, I had no basis for comparison. I assumed that every application worked this way. I assumed that state was kept in files, that coordination emerged from code order, that keys could be bound to hardware without a TPM, that databases were optional, and that sync was simply a matter of comparing timestamps.
...
This experience taught me that constraints are not obstacles. They are **design tools**. When you cannot use a database, you learn what files can do. When you cannot use a TPM, you learn how to derive a fingerprint from hardware. When you cannot rely on a network, you learn how to make the system work offline. When you cannot use a complex sync protocol, you learn how to use timestamps and UUIDs to resolve conflicts.
...
- **Sync can be simple.** When you have UUID permanence, conflict resolution becomes a matter of comparing timestamps, not implementing complex algorithms.
...
The result is an architecture that contradicts many established assumptions. It is non‑monolithic, stateless, artifact‑driven, emergent, and includes a deterministic sync mechanism that produces linear history without merge commits. It works. It has never lost data. It runs on any machine with Python and Git.
