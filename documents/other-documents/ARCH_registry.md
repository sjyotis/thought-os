# Code, Artifacts, and Their Relationships in Thought OS
...
|-----------|--------------|-----------|------|
| **Master Registry** | `NoteManager` (in `thought_os_core.py`) | `notebooks_registry.json` | Maps system fingerprint → notebook UUIDs; notebook UUID → (vault name, entry UUID, folder path). |
