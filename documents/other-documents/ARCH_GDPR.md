# Thought OS: Architectural Compliance
...
Thought OS is a local-first writing environment.
...
- Import in Thought OS (or read directly as JSON)
...
Thought OS itself is compliant by architecture. However, users may choose to:
...
These choices introduce their own compliance considerations. Thought OS does not:
...
Thought OS can be deployed in regulated environments:
- Healthcare (HIPAA)
- Finance (GLBA, SOX)
- Government (FedRAMP)
- Education (FERPA)
...
This compliance applies only to Thought OS itself.
