...
This script builds cross‑platform executables for Thought OS using Docker. It runs on Linux, macOS, or Windows and produces binaries for all target platforms without requiring native build environments.
...
|----------|--------------|-------------|--------------|
| Linux | x86_64 | `thought-os-linux` | Docker (python:3.13-slim) |
| Windows | x86_64 | `thought-os-windows.exe` | Docker (python:3.13-slim) |
| macOS | x86_64 | `thought-os-macos` | Native PyInstaller (fallback to Docker) |
...
docker run --rm -v $(pwd):/src -w /src python:3.13-slim bash -c "
    ...
    pyinstaller --clean --onefile ... thought_os.py
"
```
...
```
dist/
├── thought-os-linux          (Linux executable)
├── thought-os-windows.exe    (Windows executable)
└── thought-os-macos          (macOS executable)
...
The script is part of the Thought OS repository:

- **Location:** `build.py` or `_docker_universal_builder.py`
- **License:** Same as Thought OS (Eternal License)
